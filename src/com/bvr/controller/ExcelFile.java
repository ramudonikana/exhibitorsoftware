...
public class ExcelFile extends HttpServlet {
 ...
 Connection con = ConnectionProvider.getConnection(); 
			...
			String bb1=request.getParameter("dat");
			String bb2=request.getParameter("dtdrtname");
			 PreparedStatement pstmt;
			 ResultSet rs;
			 pstmt = con.prepareStatement("Select * from collection  WHERE da BETWEEN ? AND ? && tname=?"); 
			...
			rowhead.createCell((short) 5).setCellValue("totalaudiance");
			rowhead.createCell((short) 6).setCellValue("totalgross");
			rowhead.createCell((short) 7).setCellValue("collectionmorning");
			rowhead.createCell((short) 8).setCellValue("collectionmatinee");
			rowhead.createCell((short) 9).setCellValue("collectionfirst");
			rowhead.createCell((short) 10).setCellValue("collectionsecond");
			rowhead.createCell((short) 11).setCellValue("Collection Special");
			rowhead.createCell((short) 12).setCellValue("Profit/Loss");
			int index=1;
			while(rs.next()){
			...
			row.createCell((short)5).setCellValue(rs.getFloat("totalaudiance"));
			row.createCell((short)6).setCellValue(rs.getFloat("totalgross"));
			row.createCell((short)7).setCellValue(rs.getFloat("collectionmorning"));
			row.createCell((short)8).setCellValue(rs.getFloat("collectionmatinee"));
			row.createCell((short)9).setCellValue(rs.getFloat("collectionfirst"));
			row.createCell((short)10).setCellValue(rs.getFloat("collectionsecond"));
			row.createCell((short) 11).setCellValue(rs.getFloat("collectionspecial"));
			...
			row.createCell((short) 12).setCellValue(totalnet);
			index++;
			}
			Random randomGenerator = new Random();
		      int randomInt = randomGenerator.nextInt();
			FileOutputStream fileOut = new FileOutputStream("F:\\"+request.getParameter("tname")+"Theator Day To Day Report"+randomInt+".xls");
			wb.write(fileOut);
			try{
			  ...
			  // -- Set the FROM and TO fields --
			     ...
			     DataSource source =  new FileDataSource("F:\\"+request.getParameter("tname")+"Theator Day To Day Report"+randomInt+".xls"); // your file
			     msg.setDataHandler( new DataHandler(source));
			     msg.setFileName("F:\\"+request.getParameter("dtdrtname")+"Theator Day To Day Report"+randomInt+".xls");
			...
			response.sendRedirect("report1.jsp?bb="+bb+","+bb1+","+bb2);
			}
			catch(Exception e){}
			}
